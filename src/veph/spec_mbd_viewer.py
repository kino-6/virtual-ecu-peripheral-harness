...
def export_spec_mbd_viewer(
    ...
    return "\n".join(
        [
            ...
            "      </div>",
            "    </section>",
            _one_minute_panel(report, spec_graph, mbd_graph, interactive_context),
...
def _one_minute_panel(
    report: SpecMbdAlignmentReport,
    spec_graph: SemanticGraph,
    mbd_graph: SemanticGraph,
    context: InteractiveReviewContext | None,
) -> str:
    gap_count = (
        len(report.missing_nodes)
        + len(report.extra_nodes)
        + len(report.missing_edges)
        + len(report.extra_edges)
    )
    behavior = _compact_behavior_summary(context, report)
    return "\n".join(
        [
            '    <section class="panel compact-review">',
            "      <h2>One-Minute Review</h2>",
            "      <table class=\"compact-table\">",
            "        <thead><tr><th>Question</th><th>Evidence</th><th>Status</th></tr></thead>",
            "        <tbody>",
            f"          <tr><td>Spec to MBD alignment</td><td>{len(report.matched_nodes)} nodes / {len(report.matched_edges)} edges matched</td><td>{'PASS' if report.passed else 'FAIL'}</td></tr>",
            f"          <tr><td>Review behavior</td><td>{escape(behavior)}</td><td>{'ready' if report.passed else 'check gaps'}</td></tr>",
            f"          <tr><td>Graph shape</td><td>Spec {len(spec_graph.nodes)} nodes, MBD {len(mbd_graph.nodes)} nodes</td><td>{'same gaps: 0' if gap_count == 0 else 'gaps: ' + str(gap_count)}</td></tr>",
            "        </tbody>",
            "      </table>",
            "    </section>",
        ]
...
def _compact_behavior_summary(
    ...
    if context is not None:
        return (
            ...
            f"-> {context.output_name} {context.true_output}/{context.false_output}"
        )
    decisions = [node for node in report.matched_nodes if node.endswith("?")]
    outputs = [node for node in report.matched_nodes if node.startswith("Output ")]
    parts = []
    if decisions:
        parts.append(", ".join(decisions[:1]))
    if outputs:
        parts.append("outputs: " + ", ".join(_output_value(node) for node in outputs[:2]))
    return " / ".join(parts) or "static alignment evidence"


def _output_value(node: str) -> str:
    return node.removeprefix("Output ")
