...
def export_spec_mbd_viewer(
    ...
    return "\n".join(
        [
            ...
            f"        <span>Review: {'PASS' if report.passed else 'CHECK'}</span>",
            "        <small>Script-generated by <code>python -m veph render-spec-mbd</code>; no LLM runtime is required.</small>",
            "      </div>",
            "    </section>",
            _one_minute_panel(report, converted_model, interactive_context),
...
def _interactive_context(model: MbdModelIR) -> InteractiveReviewContext | None:
    ...
    if comparison.kind != "comparison":
        return None
    input_name, parameter_name = primary_condition_terms(model, comparison)
    ...
    return InteractiveReviewContext(
        ...
        source=source_for_signal(model.flows, input_name) or "ScenarioInput",
...
def _one_minute_panel(
    report: SpecMbdAlignmentReport,
    model: MbdModelIR,
    context: InteractiveReviewContext | None,
) -> str:
    rows = "\n".join(_behavior_case_row(row, report.passed) for row in _behavior_cases(model, context))
    return "\n".join(
        [
            '    <section class="panel compact-review">',
            "      <h2>One-Minute Review</h2>",
            "      <table class=\"compact-table\">",
            "        <thead><tr><th>Case</th><th>Stimulus</th><th>Expected</th><th>Status</th></tr></thead>",
            "        <tbody>",
            rows,
            "        </tbody>",
            "      </table>",
...
def _behavior_cases(
    ...
    if context is not None:
        parameter = _number_or_none(context.parameter_default)
        if parameter is not None:
            below = max(0, parameter - 1)
            at_value = parameter
            return [
                (
                    "below threshold",
                    f"{context.input_name}={below}",
                    _context_expected(context, below, parameter),
                ),
                (
                    "at threshold",
                    f"{context.input_name}={at_value}",
                    _context_expected(context, at_value, parameter),
                ),
            ]
        return [
            (
                "condition true",
                f"{context.input_name} {context.operator} {context.parameter_name}",
                f"{context.output_name}={context.true_output}; state={context.true_state}",
            ),
            (
                "condition false",
                f"not ({context.input_name} {context.operator} {context.parameter_name})",
                f"{context.output_name}={context.false_output}; state={context.false_state}",
            ),
        ]
    rows: list[tuple[str, str, str]] = []
    for control in model.controls[:4]:
        rows.append(
            (
                control.name,
                _condition_with_values(model, control.condition),
                _actions_with_values(model, control.actions),
            )
        )
    return rows or [("static evidence", "review graph", "see alignment evidence")]


def _behavior_case_row(row: tuple[str, str, str], passed: bool) -> str:
    case, stimulus, expected = row
    return (
        "          <tr>"
        f"<td>{escape(case)}</td>"
        f"<td>{escape(stimulus)}</td>"
        f"<td>{escape(expected)}</td>"
        f"<td>{'PASS' if passed else 'CHECK'}</td>"
        "</tr>"
    )


def _context_expected(context: InteractiveReviewContext, input_value: int, parameter_value: int) -> str:
    matched = _evaluate_operator(input_value, parameter_value, context.operator)
    output = context.true_output if matched else context.false_output
    state = context.true_state if matched else context.false_state
    return f"{context.output_name}={output}; state={state}"


def _evaluate_operator(left: int, right: int, operator: str) -> bool:
    if operator == ">=":
        return left >= right
    if operator == ">":
        return left > right
...
def _number_or_none(value: str) -> int | None:
    try:
        return int(value)
    except ValueError:
        return None
...
def _condition_with_values(model: MbdModelIR, condition: str) -> str:
    text = condition
...
def _actions_with_values(model: MbdModelIR, actions: dict[str, str]) -> str:
    values: list[str] = []
    for name, value in actions.items():
        display = model.component.parameters[value].default if value in model.component.parameters else value
        values.append(f"{name}={display}")
    return ", ".join(values)
...
def _graph_panel(title: str, graph: SemanticGraph) -> str:
    return "\n".join(
        [
            ...
            _graph_svg(graph),
            f"      <p class=\"evidence-note\">{len(graph.nodes)} semantic node(s), {len(graph.edges)} edge(s). Edge details are kept in the alignment report, not duplicated here.</p>",
            "    </section>",
        ]
    )
...
def _alignment_panel(report: SpecMbdAlignmentReport) -> str:
    return "\n".join(
        [
            ...
            f"      <p>Result: <strong>{'PASS' if report.passed else 'FAIL'}</strong>. Matched {len(report.matched_nodes)} node(s) and {len(report.matched_edges)} edge(s).</p>",
            _list_block("Key matched nodes", _key_matched_nodes(report.matched_nodes)),
            ...
            "      <p class=\"evidence-note\">Matched edge details are omitted from this compact HTML view. Use the Markdown alignment report for exhaustive trace listings.</p>",
            "    </section>",
        ]
    )


def _key_matched_nodes(values: tuple[str, ...]) -> tuple[str, ...]:
    ranked = [
        value
        for value in values
        if value.startswith("Input Port:")
        or value.startswith("Parameter:")
        or value.startswith("Constant:")
        or value.startswith("Output ")
        or value.endswith("?")
    ]
    return tuple(ranked[:8]) or values[:4]
...
def _css() -> str:
    return "\n".join(
        [
            ...
            "    .evidence-note { margin: 8px 0 0; color: var(--muted); }",
